...
test('persists real cookie identity, syncs same-context tabs, and isolates a fresh context', async ({ browser }) => {
  ...
  await first.goto('/');
  await expect(first.getByText('Đã đồng bộ')).toBeVisible({ timeout: syncTimeout });
  await second.goto('/');
  await expect(first.getByText('Đã đồng bộ')).toBeVisible({ timeout: syncTimeout });
  await expect(second.getByText('Đã đồng bộ')).toBeVisible({ timeout: syncTimeout });
  ...
  await expect(isolatedPage.getByText('Đã đồng bộ')).toBeVisible({ timeout: syncTimeout });
